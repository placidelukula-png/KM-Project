...
session["user"] = phone
session["membertype"] = member[2]   # index membertype
# session.permanent = True  # active PERMANENT_SESSION_LIFETIME (comment from STUDIO EDITOR)
...
<body>
  <div class="top">
    ...
    <div class="hdr">
      ...
      <div class="muted">membre connecté : <b>{{ phone }}</b> — <b>{{ fullname }}</b></div>
    </div>
    <div class="actions">
      <span class="pill">Rôle: <b>{{ role }}</b></span>
...
#   ## (le code que tu as déjà, c’est ici que ça vit)
    ...
    return render_template_string(PAGE, rows=rows, edit_row=None, edit_birthdate="",
